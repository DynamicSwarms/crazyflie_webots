...
class Gateway(Node):
    def __init__(self):
        super().__init__("crazyflie_webots_gateway", automatically_declare_parameters_from_overrides=True)

        self.add_service = self.create_service(WebotsCrazyflie, "~/add_crazyflie", self.add_crazyflie)
        self.remove_service = self.create_service(WebotsCrazyflie, "~/remove_crazyflie", self.remove_crazyflie)
    ...
    async def create_cf(self, id):
        ...
        description_path = os.path.join(get_package_share_directory('crazyflie_webots'),  'resource', 'cf.urdf')
        ...
        cmd = "WEBOTS_HOME={} ".format(webots_home) + webots_controller_path + " --robot-name=" + robot_name + " --protocol=tcp --ip-address=" + str(webots_ip) + " --port=1234 ros2 --ros-args -p robot_description:=" + description_path
